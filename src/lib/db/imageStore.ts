...
export async function deleteImage(id: string): Promise<void> {
	const db = await getDb();
	await Promise.all([db.delete("generated-images", id), deleteMediaFile("images", id)]);
	revokeMediaObjectUrl("images", id);
}

export async function clearImages(): Promise<void> {
	const db = await getDb();
	await Promise.all([db.clear("generated-images"), clearMediaFolder("images")]);
}
...
export function setCostCache(totalCost: number, count: number): void {
	try {
		if (typeof localStorage !== "undefined") localStorage.setItem(COST_CACHE_KEY, JSON.stringify({ totalCost, count }));
	} catch {
		/* ignore */
	}
}
